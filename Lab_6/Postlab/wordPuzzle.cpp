  			...
  			for(int currCol=0; currCol<columns; currCol++){
  				for(int currDir=0; currDir<8; currDir++){
  					for(int length=3; length<=22; length++){
  						found = getWordInGrid(currRow, currCol, currDir, length, rows, columns);
  						if(found.length()==length && dictionary.check(found)){	
  							string result = direction[currDir] + "(" + to_string(currRow) + ", " + to_string(currCol) + "):  " + found;
  							cout << result << endl;
  							wordCount++;	
  						}
  					}
