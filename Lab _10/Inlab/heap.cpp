...
void binary_heap::insert(heapNode* node) {
...
void binary_heap::percolateUp(int hole) {
    ...
    int x = heap[hole]->freq;
    // while we haven't run off the top and while the
    // value is less than the parent...
    for ( ; (hole > 1) && (x < heap[hole/2]->freq); hole /= 2) {
...
heapNode* const binary_heap::deleteMin() {
...
void binary_heap::percolateDown(int hole) {
 ...
 heapNode* const binary_heap::findMin() {
    if (heap_size == 0) {
